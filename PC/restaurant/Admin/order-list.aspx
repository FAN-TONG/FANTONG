...
<div class="page-container">
      ...
      <%
          if (Session["user"] == null)
        {
          ...
          string sql=@"select dish.name,[table].id,waiter.name,cook.name,member.account,[order].status,[order].time,[order].id from
                    [order]
                    left outer join cook on([order].cookId=cook.id)
                    left outer join [table] on([order].tableId=[table].id)
                    left outer join dish on([order].dishId=dish.id)
                    left outer join waiter on([order].waiterId=waiter.id)
                    left outer join member on([order].memberId=member.account)";
			...
			<tbody>
				...
				<tr class="text-c"> 
					...
					<td class="td-status"><span class="label label-success radius">
                        <%
                            int status = Convert.ToInt32(dt.Rows[i][5]);
                            if(status==0)
                                Response.Write("已点未做");
                            else if(status==1)
                                Response.Write("已做未上");
                                ...
                                Response.Write("已上");
                            %></span></td>
                    <td><%Response.Write(dt.Rows[i][6]);%></td>
					<td class="td-manage">
                     <a style="text-decoration:none" class="ml-5" onClick="picture_del(this,'<%Response.Write(dt.Rows[i][7].ToString()); %>')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
				</tr>
                <%} %>
			</tbody>
		</table>
